...
describe Chess::MoveGenerator::Sliding do
  ...
  context 'Queen moves' do
    def make_board_with_queen(square)
      Chess::Board.new(
        white_pawns: 0,
        white_knights: 0,
        white_bishops: 0,
    ...
    it 'generates orthogonal and diagonal queen moves on empty board' do
      board = make_board_with_queen(27)
      moves = Chess::MoveGenerator::Sliding.generate_queen_moves(board, Chess::WHITE)
      # expect all moves from rook and bishop tests to be generated
      expect(moves.any? { |m| m.from == 27 && m.to == 48 }).to be true
      expect(moves.any? { |m| m.from == 27 && m.to == 9 }).to be true
      expect(moves.any? { |m| m.from == 27 && m.to == 54 }).to be true
      expect(moves.any? { |m| m.from == 27 && m.to == 6 }).to be true
    ...
    it 'generates orthogonal and diagonal queen moves with blockers' do
      board = make_board_with_queen(27)
      board.add_piece(Chess::BLACK, :rook, 41)
      board.add_piece(Chess::WHITE, :rook, 20)
      board.add_piece(Chess::BLACK, :bishop, 43)
